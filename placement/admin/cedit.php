<?php
//companybase Connection
include 'config.php';
//Get ID from companybase
if(isset($_GET['edit_id'])){
 $sql = "SELECT * FROM company WHERE id =" .$_GET['edit_id'];
...
if(isset($_POST['btn-update'])){
 ...
 $update = "UPDATE company SET username='$username', webaddress='$webaddress',address='$address',phoneno='$phoneno',date='$date',password='$password' WHERE id=". $_GET['edit_id'];
...
<head>
        ...
        <style type="text/css">
          ...
          .tab{width:300px; height:50px; margin: auto;margin-top:45px; border-radius: 30px;}
...
<h3>Enter the all details agian </h3>
  <div style="padding-bottom:5px;"><a href="companyTable.php">Company List</a></div>
<table  align="center" style="border-radius:30px; width:40%;" class="box">
  <tr><td><label>COMPANY NAME:</label></td><td><input type="text" name="username" value="<?php echo $row['username']; ?>" placeholder="Name" class="box" ></td></tr>
  <tr><td><label>WEB ADDRESS:</label></td><td><input type="text" name="webaddress" value="<?php echo $row['webaddress']; ?>" placeholder="webaddress" class="box" ></td></tr>
  <tr><td><label>ADDRESS:</label></td><td><input type="text" name="address" value="<?php echo $row['webaddress']; ?>" placeholder="address" class="box" ></td></tr>
  <tr><td><label>PHONE NUMBER:</label></td><td><input type="text" name="phoneno" value="<?php echo $row['phoneno']; ?>" placeholder="phoneno" class="box"></td></tr>
  <tr><td><label>DATE:</label></td><td><input type="date" name="date" value="<?php echo $row['date']; ?>" placeholder="date" class="box" ></td></tr>
  <tr><td><label>PASSWORD:</label></td><td><input type="password" name="password" value="<?php echo $row['password']; ?>" placeholder="password" class="box" ></td></tr>
  <tr><td><button type="submit" class="ln btn btn-primary" name="btn-update" id="btn-update" onClick="update()"><strong>Update</strong></button></td><td><a href="cview.php"><button type="button" class="ln btn btn-primary" value="button">Cancel</button></a></td></tr>
</table>
<!-- Alert for Updating -->
<script>
function update(){
